...
namespace
{
	float SmoothRegionRange(float Start, float End, float Value)
	{
	...
	float ValueNoise(const FVector& Position, float CellSize, int32 Seed)
	{
		...
		const float AlphaX = SmoothRegionRange(0.0f, 1.0f, GridX - X0);
		const float AlphaY = SmoothRegionRange(0.0f, 1.0f, GridY - Y0);
...
namespace UEGT1WorldLayout
{
	...
	int32 GetMinTileX() { return -GetTileRadius() - UUEGT1RegionSettings::Get().WestTileExtension; }
	int32 GetMaxTileX() { return GetTileRadius(); }
	int32 GetMinTileY() { return -GetTileRadius(); }
	int32 GetMaxTileY() { return GetTileRadius(); }
	int32 GetExpectedTileCount() { return (GetMaxTileX() - GetMinTileX() + 1) * (GetMaxTileY() - GetMinTileY() + 1); }
	float GetTileSize() { return UUEGT1RegionSettings::Get().TileSize; }
	float GetWorldMinX() { return (GetMinTileX() - 0.5f) * GetTileSize(); }
	float GetWorldMaxX() { return (GetMaxTileX() + 0.5f) * GetTileSize(); }
	float GetWorldMinY() { return (GetMinTileY() - 0.5f) * GetTileSize(); }
	float GetWorldMaxY() { return (GetMaxTileY() + 0.5f) * GetTileSize(); }
	float GetWorldHalfExtent() { return FMath::Max(FMath::Abs(GetWorldMinX()), GetWorldMaxX()); }
	...
	FUEGT1RegionSample SampleRegion(const FVector& WorldPosition)
	{
		...
		const FVector TownSpineStart(-Settings.TownWestExtension, 0.0f, 0.0f);
		const FVector ClosestTownPoint = FMath::ClosestPointOnSegment(WorldPosition, TownSpineStart, FVector::ZeroVector);
		const float TownDistance = FVector::Dist2D(WorldPosition, ClosestTownPoint);
		const float TownWeight = 1.0f - SmoothRegionRange(Settings.TownCoreRadius, Settings.TownBlendRadius, TownDistance);
		const float NonTown = 1.0f - TownWeight;
		const float Farmland = SmoothRegionRange(Settings.DirectionalBiomeStart, Settings.DirectionalBiomeFull, -WorldPosition.X);
		const float Highlands = SmoothRegionRange(Settings.DirectionalBiomeStart, Settings.DirectionalBiomeFull, WorldPosition.Y);
		const float Tropical = SmoothRegionRange(Settings.DirectionalBiomeStart, Settings.DirectionalBiomeFull, -WorldPosition.Y);
		const float Ocean = SmoothRegionRange(Settings.OceanStart, Settings.OceanFull, WorldPosition.X);
		const float Coast = SmoothRegionRange(Settings.CoastStart, Settings.OceanStart, WorldPosition.X) * (1.0f - Ocean);
		...
		const float NorthProgress = SmoothRegionRange(Settings.DirectionalBiomeStart, Settings.DirectionalBiomeFull, WorldPosition.Y);
		const float OceanProgress = SmoothRegionRange(Settings.CoastStart, Settings.OceanFull, WorldPosition.X);
	...
	float GetTemperatureCelsius(const FVector& WorldPosition, float HourOfDay)
	{
		const FUEGT1RegionSample Region = SampleRegion(WorldPosition);
		const float ClimateTemperature = 3.0f + Region.Temperature * 23.0f;
		const float DiurnalTemperature = FMath::Sin((HourOfDay - 8.0f) / 24.0f * 2.0f * PI) * 4.5f;
		return ClimateTemperature + DiurnalTemperature;
	}
	...
	bool IsReservedGameplaySpace(const FVector& WorldPosition, float Radius)
	{
		const UUEGT1RegionSettings& Settings = UUEGT1RegionSettings::Get();
		const FVector TownSpineStart(-Settings.TownWestExtension, 0.0f, 0.0f);
		const FVector ClosestTownPoint = FMath::ClosestPointOnSegment(WorldPosition, TownSpineStart, GetSanctuaryLocation());
		if (FVector::Dist2D(WorldPosition, ClosestTownPoint) < Settings.TownReserveRadius + Radius)
		{
			return true;
		}
